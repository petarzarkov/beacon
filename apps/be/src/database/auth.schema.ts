...
export const account = sqliteTable('account', {
  id: text().primaryKey(),
  /**
   * Who vouched for this identity. Added in better-auth 1.7, which scopes an
   * account by issuer as well as by provider - so `credential` from two sources
   * is two accounts rather than one collision.
   */
  issuer: text().notNull(),
